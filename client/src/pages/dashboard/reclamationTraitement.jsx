...
export default function ReclamationTraitement() {
  ...
  useEffect(() => {
    ...
    axios
      ...
      axios
      .put(`http://localhost:8080/demandeReclamation/updateStatusDoing/${reclamation.id}`)
      .then((res) => { 
      });
  }, []);
  ...
  const hundleAccept=()=>{
      ...
      axios
      .post(`http://localhost:8080/Email/sendEmail`,send)
      .then((res) => {  
      });
      axios
      .put(`http://localhost:8080/demandeReclamation/updateStatusDoneWithAccepted/${reclamation.id}`)
      .then((res) => { 
      });
}
  const hundleDecline=()=>{
    const send={
      ...
      axios
      .put(`http://localhost:8080/demandeReclamation/updateStatusDoneWithDeline/${reclamation.id}`)
      .then((res) => {
        setReclamation(res.data);
        console.log(res.data);
      });
  ...
  return (
    <div className="container mx-auto mt-8 p-8 bg-gray-100 rounded-md">
      ...
      <div className="mb-4 gap-7 flex flex-row justify-center items-center">
        <button onClick={()=>{hundleAccept();navigate("../traitement")}} className="bg-yellow-700 hover:bg-yellow-800 rounded-lg px-5 py-2 text-white">Accept</button>
        <button onClick={()=>{hundleDecline();navigate("../traitement")}} className="bg-yellow-700 hover:bg-yellow-800 rounded-lg px-5 py-2 text-white">Decline</button>
      </div>
    </div>
  );
}
